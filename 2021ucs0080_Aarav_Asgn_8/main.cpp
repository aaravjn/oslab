/*
You need to implement variable partitioning memory management
We shall assume that 500 MB memory is available and each program requires an integer multiple of 1 MB memory

Memory manager receives requests for memory allocation for programs. 
...
If a process can not be allocated space, it shall be queued in waiting. 
After all requests are served, print the output of waiting queue for each management policy.
*/
